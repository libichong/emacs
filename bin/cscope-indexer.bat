...
REM Awk, Dos batch, COBOL, C, C++, C#, Erlang, Fortran, Java, JavaScript, Lisp, Lua, Pascal, Perl, PHP, Python, Ruby, Matlab, OCaml, Scheme, Tcl, TeX, Verilog, Vhdl and Vim.
grep -i -E "\.([chly](xx|pp)*|cc|hh|cs|java|py|el|js|php|go)$" %LIST_FILE%.tmp | sed -e "s/^/\"/;s/$/\"/" > %LIST_FILE%
cscope -b -i %LIST_FILE% -f %DATABASE_FILE%
goto DONE

:ALMOST_DONE
grep -i -E "\.([chly](xx|pp)*|cc|hh|cs|java|py|el|js|php|go)$"  %LIST_FILE%.tmp > %LIST_FILE%
goto DONE
